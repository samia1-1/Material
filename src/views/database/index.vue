...
export default {
  ...
  methods: {
    ...
    menuFun(){
      let menuD=[];
        ...
        for(let i=0;i<keyList.length;i++){
          let url= "http://192.168.31.63:8100/json/"+keyList[i]+".json"
          let num=0;
    ...
    changeFun(name,data){
      ...
      // let getJsonUrl="http://192.168.31.63:8100/json/"+data.name+".json"
    ...
    //获取元素
    getysFun(){
      let url= "http://192.168.31.63:8100/json/K423.json"
      let list=[]
      let componentList=[]
      getJson(url).then(data => {
    ...
    async asJson(){
      ...
      console.log(sheetNames2)
       ...
       //获取table数据
      let url= "http://192.168.31.63:8100/json/source/19_K423.json"
      getJson(url).then(data => {
          //table处理
